...
-   **Development Dependencies (`[dependency-groups]`)**: These are your "workshop tools"—packages used only for development, testing, and analysis. Examples include `pytest` (for testing), `ruff` (for linting), or `ipykernel` (for notebooks). Dependency groups are the standard way (added by `uv add --group dev`) to keep these out of a production build, keeping it lean and secure.
...
To keep these commands consistent across your machine, your teammates, git hooks, and CI, this course wraps them as [`mise`](https://mise.jdx.dev/) tasks. Rather than memorizing each `uv run` invocation, you rely on a shared vocabulary—`mise run install`, `mise run format`, `mise run check`, `mise run test`, and `mise run build`—and each task simply delegates to the matching `uv run` command under the hood. You will set this up in the [Task Automation section](../5. Refining/5.1. Task Automation.md).
