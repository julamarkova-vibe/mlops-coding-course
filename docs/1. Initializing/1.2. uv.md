...
This command automatically finds your system's Python and creates a virtual environment. To use a specific Python version, use the `--python` flag (e.g., `uv venv --python 3.14`).
...
This creates a `.python-version` file in your project's root directory (containing, e.g., `3.14`). `uv` will automatically respect this file for all subsequent commands, keeping your development and production versions aligned.
