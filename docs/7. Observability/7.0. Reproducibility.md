...
To achieve this with `uv`, you can define a constrained build process as `mise.toml` tasks:

```toml
# mise.toml — deterministic build tasks

[tasks."package:constraints"]
description = "Pin and hash every dependency from pyproject.toml"
run = "uv pip compile pyproject.toml --generate-hashes --output-file=constraints.txt"

[tasks.package]
description = "Build a reproducible wheel using the pinned constraints"
depends = ["package:constraints"]
run = "uv build --build-constraint=constraints.txt --require-hashes --wheel"
```

Run it with `mise run package`. The `--build-constraint` flag forces `uv` to use the exact dependency versions specified in `constraints.txt`, while `--require-hashes` validates that each package matches its expected hash, preventing any variation.
...
Rather than declaring a separate `python_env.yaml`, the project reuses the `uv`-managed environment at runtime via `--env-manager=local` (shown below), so there is only one environment definition to maintain.
...
```bash
mlflow run --env-manager=local --experiment-name=bikes --run-name=Training -P conf_file=confs/training.yaml .
```

This command executes the project located in the current directory (`.`), reuses the local `uv`-managed environment (`--env-manager=local`), and passes the training configuration file to the main entry point. In practice, this is wrapped as a `mise` task (`mise run project:run training`) so the same command is shared by hooks and CI.
...
- **[MLflow Project execution (mise tasks) from the MLOps Python Package](https://github.com/fmind/mlops-python-package/blob/main/mise.toml)**
