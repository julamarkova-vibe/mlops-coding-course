...
First, configure MLflow to know where to store its data. For local development, you can point both the tracking and registry URIs to a local directory. MLflow 3 puts the filesystem store in maintenance mode, so opt in with `MLFLOW_ALLOW_FILE_STORE=true`; in production, prefer a database backend such as `sqlite:///mlflow.db`.
...
# MLflow 3 requires opt-in for the filesystem store during local development.
...
with mlflow.start_run(run_name="training") as run:
    ...
    # MLflow 3 uses `name=` for the model path (the old `artifact_path=` is deprecated).
...
mlflow.sklearn.log_model(
    model,
    name="model", # MLflow 3 replaces the deprecated `artifact_path=` with `name=`
    signature=signature,
...
You can interact with the registry using the `MlflowClient` to search for models, retrieve versions, and inspect their aliases. Note that MLflow 3 **removed the legacy model "stages"** (Staging, Production) in favor of aliases and tags, so query those instead of `current_stage`.
...
for mv in model_versions:
    print(f"Version: {mv.version}, Aliases: {mv.aliases}, URI: {mv.source}")
```
...
Before promoting a candidate, validate that it meets your quality bar. MLflow 3 split threshold validation out of `mlflow.evaluate()` into a dedicated [`mlflow.validate_evaluation_results()`](https://mlflow.org/docs/latest/python_api/mlflow.html#mlflow.validate_evaluation_results) call, which raises if the metrics fall short:

```python
import mlflow
from mlflow.models import MetricThreshold

# Evaluate the candidate model, then validate it against your thresholds
results = mlflow.evaluate(model=model_uri, data=eval_data, model_type="regressor")
mlflow.validate_evaluation_results(
    candidate_result=results,
    baseline_result=None,
    validation_thresholds={
        "r2_score": MetricThreshold(threshold=0.5, greater_is_better=True),
    },
)
```
