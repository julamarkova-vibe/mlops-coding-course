...
To learn more, consult the official [typing module](https://docs.python.org/3/library/typing.html) documentation and the [type hints cheat sheet](https://mypy.readthedocs.io/en/stable/cheat_sheet_py3.html). Type hints follow the same standard syntax regardless of which type checker you use to enforce them.
...
- [Mypy](https://mypy-lang.org/): The original and most mature Python type checker, with a broad plugin ecosystem.
...
All of these tools read the same standard type hints, so you can adopt annotations today and swap checkers later with little friction.

## How can you configure ty for your project?

You configure `ty` by adding a `[tool.ty]` section to your `pyproject.toml` file, so every developer and CI run share the same rules. Remember to add the `.ty_cache/` directory to your `.gitignore` file.

`ty` infers the target Python version from `requires-python`, but you can set it explicitly. You can also tune individual diagnostics: because `ty` is pre-1.0, it does not yet fully model the dynamic typing of some machine learning libraries (Pydantic, Pandera, MLflow). Relaxing the categories they trigger keeps `ty` focused on catching real errors, such as undefined names and bad calls.

```toml
[tool.ty.environment]
# Target Python version for type resolution.
python-version = "3.14"
...
```python
dataset = mlflow.data.from_pandas(df)  # ty: ignore[unresolved-attribute]
```

Run `uv run ty explain rule <name>` to understand a diagnostic before silencing it. For all options, refer to the [ty documentation](https://docs.astral.sh/ty/) and its [rules reference](https://docs.astral.sh/ty/reference/rules/).
...
- **Integrate Type Checking into CI/CD**: Run `ty` as part of your continuous integration pipeline to catch type errors before they reach production.
